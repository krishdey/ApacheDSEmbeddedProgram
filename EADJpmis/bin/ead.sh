...
EAD_OUT="$EAD_INSTANCE/log/apacheds.out"
EAD_PID="$EAD_INSTANCE/run/apacheds.pid"
touch $EAD_OUT
touch $EAD_PID
...
if [ "$EAD_ACTION" = "start" ]; then
    ...
    [ $HAVE_TTY -eq 1 ] && echo "Starting ApacheDS instance '$EAD_INSTANCE_NAME'..."
    ...
    # Launching ApacheDS
     eval "\"$RUN_JAVA\"" \
     -Dlog4j.configuration="\"file:$EAD_INSTANCE/conf/log4j.properties\"" \
     -Dapacheds.log.dir="\"$EAD_INSTANCE/log\"" \
    ...
    # Printing instance information
    [ $HAVE_TTY -eq 1 ] && echo "Running ApacheDS instance '$EAD_INSTANCE_NAME'..."
      eval "\"$RUN_JAVA\"" \
    ...
    if [ -f $EAD_PID ]; then
        PID=`cat $EAD_PID`
        [ $HAVE_TTY -eq 1 ] && echo "Stopping ApacheDS instance '$EAD_INSTANCE_NAME' running as $PID"
        ...
        while [ $ATTEMPTS_REMAINING > 0 ]; do
            ...
            if [ $? > 0 ]; then
                ...
                [ $HAVE_TTY -eq 1 ] && echo "ApacheDS instance '$EAD_INSTANCE_NAME' stopped successfully"
                break
            fi
            sleep 1
        ...
        [ $HAVE_TTY -eq 1 ] && echo "ApacheDS is not running, $EAD_PID does not exist"
    fi
fi
